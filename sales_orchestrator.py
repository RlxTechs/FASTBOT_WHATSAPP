...
def has_campaign_context(state: Dict) -> bool:
    return bool(state.get("campaign_id") or state.get("campaign_label"))

def is_generic_facebook_more_info(msg: str) -> bool:
    m = normalize(msg)
    checks = [
...
def asks_show_catalog(msg: str) -> bool:
    ...
    checks = [
        ...
        "montrez moi", "envoyez moi vos produits"
    ]
    return any(x in m for x in checks)
...
def catalog_overview() -> str:
    return cfg().get("non_food", {}).get(
        "catalog_overview",
        "Nous proposons plusieurs produits et services. Dites-moi ce que vous cherchez."
    )

def no_thumbnail_seller_fallback() -> str:
    return cfg().get("non_food", {}).get(
        "facebook_no_context_seller_fallback",
        "Bonjour 👋 Oui bien sûr. Dites-moi ce qui vous intéresse exactement : nourriture, TV, électroménager, informatique, iPhone, énergie ou papeterie ?"
    )
